...
{
    /// <summary>
    /// This class just has a large list of strings that it hashes and sticks in a dictionary.
    /// If provided a hash, it can see if it has a string that matches it. Not all property strings are known,
    /// and so this class is here only to see try and find a string for an unknown/unregistered property.
    /// If more strings are added to this, we can hopefully find the names of the remaining properties and
    /// reverse their contents.
    /// </summary>
    public static class HashGuesser
    {
        private static Dictionary<uint, string> _hashDictionary = new Dictionary<uint, string>();
        private static Dictionary<uint, string> _hashDictionaryAlt = new Dictionary<uint, string>(); //Uses a different hashing method for classname hashes

        //Todo: Consider moving this to a file that's loaded so people can share string lists and hopefully find the remaining properties quicker
        /// <summary>
        /// List of possible property names and object class names. Used to guess names of unknown properties.
        /// Some may be wrong. That's fine. The whole point is to have as many possibly related strings as possible,
        /// to increase the chances of finding the string for an unknown property/class.
        /// </summary>
        ...
        static uint StringHash(string key)
        {
            string keyLower = key.ToLower(); //Copy the string since we want to leave the original string intact
            ...
            {
                hash = keyLower[i] & 255 ^ hash * 33;
            }
            return (uint)(hash % keyLower.Length);
        }
        ...
        {
            foreach (var name in _stringList)
            {
                _hashDictionary[name.HashVolitionCRC(0)] = name;
                _hashDictionaryAlt[name.HashVolition()] = name;
            }
        }

        public static bool TryGuessHashString(uint hash, out string name)
        {
            if (_hashDictionary.ContainsKey(hash))
            {
                name = _hashDictionary[hash];
                return true;
            }
            if (_hashDictionaryAlt.ContainsKey(hash))
            {
                name = _hashDictionaryAlt[hash];
                return true;
            }
